var RadarChart = {
  ...
  chart: function() {
    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        var  RADIANT_FLAG = 0, DIRE_FLAG = 1;  // HAR VI LAGT TILL!
        ...
        polygon.enter().append('polygon')
          ...
          .on('mouseover', function (d,i){
            if(i != RADIANT_FLAG && i != DIRE_FLAG) {   //--------------- HAR VI LAGT TILL
          ...
          circle.enter().append('circle')
            ...
            .on('mouseover', function(d, i, j){
              tooltip.transition()
                ...
                var name;
                if(j == 0 || j == 1)
                  name = (j == 0) ? "Radiant Average" : "Dire Average";
                else
                  name = roster.data[j-2].playerName;

              tooltip.html('<strong>' + name + '</strong><br>' + ~~(d[0].value*cfg.maxValues[d[0].axis]*0.1) + ' ' + d[0].axis)
                    .classed('visible', 1);

                if(j != RADIANT_FLAG && j != DIRE_FLAG) {                // ------- HAR VI LAGT TILL
                  container.classed('focus', 1);
  ...
  draw: function(id, d, options) {
    ...
    d3.select(id)
      ...
      .attr("height", cfg.h)
      .append("g")
      .attr("transform", "translate("+cfg.xOffset+","+cfg.yOffset+")" + "scale(0.85)")
